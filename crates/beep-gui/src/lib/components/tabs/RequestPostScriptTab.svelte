<script lang="ts">
    ...
    function isFilePath(v: string | null): boolean {
        if (!v) return false;
        const trimmed = v.trim();
        // Any text after < or > that isn't {% is a file path
...
<div class="flex flex-col h-full p-2 gap-2 min-h-0">
    ...
    {#if useExternalFile}
        ...
        <p class="text-xs text-base-content/50">
            Script will be run <span class="text-accent">after request is sent</span>.
        </p>
    {:else}
