...
{#if open}
    ...
    <dialog class="modal items-start" open onclose={onClose} onclick={onClose}>
        ...
        >
            <div class="px-3 py-2 text-xs uppercase tracking-wide opacity-50 font-semibold border-b border-base-content/10">
                New
            </div>
            <div class="">
                {#each items as item, i}
                    ...
                    <div
                        class="flex flex-col px-3 py-1.5 cursor-pointer {i === selectedIdx ? 'bg-primary/80' : 'hover:bg-base-300'}"
                        onclick={() => item.action()}
                        role="button"
                        tabindex="0"
                    >
